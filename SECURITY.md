...
- **Proxy headers off by default.** `TRUST_PROXY_HEADERS=true` trusts `X-Forwarded-*` only from the addresses in `FORWARDED_ALLOW_IPS`. That is inbound; for outbound module and LLM proxies see [docs/proxies.md](docs/proxies.md).
